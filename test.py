class TenderMonitoringPendingEMDEmailTriggerAPIView(APIView):
    ...
    def get(self, request):
        ...
        tender_data = TenderMasterNew.cmobjects.filter(
            ...
            financial_bid_open_date__lte=current_date - timedelta(days=31)
        ).values(
            'id', 'tender_id', 'financial_bid_open_date',
            'emd_amount', 'emd_type', 'emd_deposition_location',
            'emd_validity_date', 'emd_checked_by_account'
        )
        ...
        if to_users_list and tender_data:
            ...
            context = {
                "date": current_date,
                "data" : tender_data,
                "pending_since_foD_date" : current_date - timedelta(days=31),
            }
...
error - TypeError at /tender-auto-pending-emds-trigger/
