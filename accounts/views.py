...
class LoginView(KnoxLoginView):
	...
	def post(self, request, *args, **kwargs):
		...
		self.validate_user_type(user, user_type)
		if user_type == 'blf':
			otp = request.data.get('otp')
			if not otp:
				return Response(self.generate_login_otp(user, user_type))
			self.validate_login_otp(user, otp)
	...
	def validate_user_type(self, user, user_type):
		...
		if not profile or not profile.user_type or profile.user_type.name.lower() != user_type:
			raise APIException({
				'request_status': 0,
				'msg': f"User is not registered as {user_type}."
			})

	def generate_login_otp(self, user, user_type):
		profile = Profile.cmobjects.filter(user=user, is_allow_otp_login=True).select_related('user_type').first()
		if not profile:
			raise APIException({
				'request_status': 0,
				'msg': 'OTP login is not enabled for this user.'
			})
		expiry_time = int(os.getenv('OTP_EXPIRY', '5'))
		secret = pyotp.random_base32()
		totp = pyotp.TOTP(secret, digits=4)
		otp = totp.now()
		expiry = timezone.now() + timedelta(minutes=expiry_time)
		UserOTP.objects.create(user=user, otp_secret=secret, expiry=expiry)
		return {
			'request_status': 1,
			'otp_required': True,
			'otp': otp,
	...
	def validate_login_otp(self, user, otp):
		try:
			user_otp = UserOTP.cmobjects.filter(user=user, is_used=False).latest('created_at')
